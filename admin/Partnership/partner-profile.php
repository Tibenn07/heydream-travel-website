...
if ($rawLocationText !== '' && GOOGLE_MAPS_API_KEY !== '') {
    $mapSrc = 'https://www.google.com/maps/embed/v1/place?key=' . urlencode(GOOGLE_MAPS_API_KEY) . '&q=' . urlencode($rawLocationText);
} elseif ($rawLocationText !== '') {
    $mapSrc = 'https://www.google.com/maps?q=' . urlencode($rawLocationText) . '&output=embed';
} else {
    $mapSrc = null;
}
?>
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .status-chip.pending { background: rgba(245, 158, 11, 0.2); border-color: rgba(245, 158, 11, 0.5); color: #fef3c7; }
        .status-chip.rejected { background: rgba(239, 68, 68, 0.2); border-color: rgba(239, 68, 68, 0.5); color: #fecaca; }
...
<body>
    <div class="page-shell">
        <div class="top-bar">
            ...
            <a href="<?= $is_partner ? 'partner-dashboard.php' : '../dashboard.php' ?>" class="top-back">
            ...
            <div class="profile-hero-inner">
                <div class="profile-hero-left">
                    ...
                    <div>
                        ...
                        <p class="profile-hero-bio"><?= !empty($bio) ? $bio : (!empty($description) ? $description : 'This partner is committed to delivering memorable travel experiences for every customer.') ?></p>
                        <span class="status-chip <?= strtolower($partner['status'] ?? 'pending') ?>"><i class="fas fa-circle-check"></i> <?= $status ?> Partner</span>
                    </div>
                </div>
                <div class="profile-hero-stats">
                    <div class="profile-stat">
                        <i class="fas fa-calendar-check"></i>
                        <div class="stat-title">Business Since</div>
                        <div class="stat-value"><?= htmlspecialchars($businessSince) ?></div>
                    </div>
                    <div class="profile-stat">
                        <i class="fas fa-suitcase-rolling"></i>
                        <div class="stat-title">Packages Listed</div>
            ...
            <div>
                <div class="info-card">
                    ...
                    <?php if (!empty($specialties)): ?>
                        <div class="chip-row">
                            <?php foreach (array_filter(array_map('trim', explode(',', $specialties))) as $tag): ?>
                ...
                <div class="info-card">
                    ...
                    <div class="info-row">
                        ...
                        <div><div class="info-label">Joined</div><div class="info-value"><?= htmlspecialchars($joinedAt) ?></div></div>
                    </div>
                    <div class="info-row">
                        <div class="info-icon"><i class="fas fa-circle-check"></i></div>
                        <div><div class="info-label">Approved</div><div class="info-value"><?= htmlspecialchars($approvedAt) ?></div></div>
                    </div>
                </div>
            </div>
            <div>
                <div class="info-card">
                    ...
                    <div class="info-row">
                        ...
                        <div><div class="info-label">Location</div><div class="info-value <?= empty($locationParts) ? 'empty' : '' ?>"><?= !empty($locationParts) ? htmlspecialchars(implode(', ', $locationParts)) : 'Not provided' ?></div></div>
                    </div>
                ...
                <div class="info-card">
                    ...
                    <?php else: ?>
                        <p style="margin:0; color: var(--muted); font-size: 0.9rem;">No extra social links were added yet.</p>
                    <?php endif; ?>
                </div>
                <div class="info-card">

...
// Redirect Sales users away from Content Manager
if ($_SESSION['admin_role'] === 'sales') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'save_destination_partial') {
        ...
        try {
            // The step-wizard UI submits ALL tabs' fields in a single final
            // "Save" click (active_tab is always whatever tab happened to be
            // showing last, e.g. Gallery) -- unlike the old per-tab UI this
            // handler was written for, which saved one tab's columns per
            // request. Switching on active_tab here silently dropped every
            // other tab's data. Run every group unconditionally instead.
    ...
    elseif ($action === 'save_foreign_destination_partial') {
        ...
        if ($id <= 0) {
            ...
            // The wizard's General Info step has no dest_key input, so new
            // records were saved with dest_key='' -- every public page that
            // links to a foreign destination (homepage cards, search, "Book
            // This Deal") uses dest_key directly with no fallback, making
            // the package completely unreachable/unbookable. Auto-generate
            // one from the name (suffixed with the id for uniqueness),
            // matching the slug logic already used by the non-wizard
            // save_foreign_destination handler below.
            if (trim($_POST['dest_key'] ?? '') === '') {
                $autoKey = strtolower(str_replace([' ', '/', '\\', '&', '?'], '-', trim($_POST['name'] ?? 'destination'))) . '-' . $id;
                $pdo->prepare("UPDATE foreign_destinations SET dest_key = ? WHERE id = ?")->execute([$autoKey, $id]);
            }
        }
    ...
    elseif ($action === 'delete_destination') {
        ...
        $stmt->execute([$id]);
        $dest = $stmt->fetch();
        if ($dest && $dest['image_path'] && file_exists(__DIR__ . '/../' . $dest['image_path'])) {
            unlink(__DIR__ . '/../' . $dest['image_path']);
        }
    ...
    elseif ($action === 'delete_foreign_destination') {
        ...
        if ($dest) {
            foreach (['image_path', 'image2_path', 'image3_path'] as $img) {
                if ($dest[$img] && file_exists(__DIR__ . '/../' . $dest[$img])) {
                    unlink(__DIR__ . '/../' . $dest[$img]);
                }
            }
        }
    ...
    elseif ($action === 'delete_visa') {
        ...
        $stmt->execute([$id]);
        $visa = $stmt->fetch();
        if ($visa && $visa['icon_type'] === 'upload' && $visa['icon_value'] && file_exists(__DIR__ . '/../' . $visa['icon_value'])) {
            unlink(__DIR__ . '/../' . $visa['icon_value']);
        }
    ...
    elseif ($action === 'save_site_service') {
        try {
            ...
            if ($service_id > 0) {
                ...
                departure_date=?, return_date=?, status_text=?, is_active=?, is_featured=?, display_order=? 
                WHERE id=?");
                $stmt->execute([
                    $service_type,
                    $title,
...
<body>
    ...
    <script>
            ...
            if (missing.length > 0) {
                ...
                if (firstInvalid) {
                    ...
                    if (tabContent && !tabContent.classList.contains('active')) {
                        const tabBtn = document.querySelector(`.tab-btn[data-target="${tabContent.id}"]`);
        ...
        function updateHotel(type, index, field, value) {
            let hotelData = [];
            if (type === 'local') hotelData = localHotels;
            ...
            const hiddenInputId = type === 'local' ? 'local_hotels_json' : 'foreign_hotels_json';
            const hiddenInput = document.getElementById(hiddenInputId);
            if (hiddenInput) hiddenInput.value = JSON.stringify(hotelData);
        }
        ...
        function initStepWizard(modalId, formId) {
            ...
            nextBtn.addEventListener('click', function () {
                // Scope validation to the tab actually visible right now --
                // admin's validateRequiredFields(form) with no scope checks
                // every [required] field across ALL tabs at once (unlike
                // partner's simpler single-arg version), so without this it
                // blocked Next on step 1 for fields that only exist on a
                // later tab (e.g. Base Price on the Pricing tab).
                const currentTabContent = current < tabBtns.length ? document.getElementById(tabIdFor(tabBtns[current])) : null;
                if (current < realStepCount && !validateRequiredFields(form, currentTabContent)) return;

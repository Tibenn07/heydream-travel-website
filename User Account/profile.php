    ...
    <div class="profile-container">
        ...
        <div style="margin-top: 40px;">
            <div class="section-title" style="margin-bottom: 20px;">
                <i class="fas fa-ticket-alt" style="color: #003580;"></i>
                <h2>MY VOUCHER WALLET</h2>
            </div>

            <!-- Tab row -->
            <div style="display:flex; gap: 0; border-bottom: 2px solid #e2e8f0; margin-bottom: 24px;">
                <button id="vt-collected" onclick="switchVoucherTab('collected')"
                    style="padding: 12px 24px; border: none; background: none; font-weight: 700; font-size: 0.9rem; cursor: pointer; color: #003580; border-bottom: 2px solid #003580; margin-bottom: -2px; transition: all 0.2s;">
                    <i class="fas fa-wallet"></i> My Vouchers
                </button>
                <button id="vt-available" onclick="switchVoucherTab('available')"
                    style="padding: 12px 24px; border: none; background: none; font-weight: 600; font-size: 0.9rem; cursor: pointer; color: #94a3b8; border-bottom: 2px solid transparent; margin-bottom: -2px; transition: all 0.2s;">
                    <i class="fas fa-gift"></i> Available to Claim
                </button>
            </div>

            <!-- MY Collected Vouchers Tab -->
            <div id="voucher-tab-collected">
                <div id="collected-vouchers-container" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px;">
                    <div style="text-align: center; color: #94a3b8; padding: 40px; grid-column: 1/-1;">
                        <i class="fas fa-spinner fa-spin" style="font-size: 2rem;"></i>
                        <p style="margin-top: 12px;">Loading your vouchers...</p>
                    </div>
                </div>
            </div>

            <!-- AVAILABLE Vouchers to Claim Tab -->
            <div id="voucher-tab-available" style="display: none;">
                <div id="available-vouchers-container" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px;">
                    <div style="text-align: center; color: #94a3b8; padding: 40px; grid-column: 1/-1;">
                        <i class="fas fa-spinner fa-spin" style="font-size: 2rem;"></i>
                        <p style="margin-top: 12px;">Loading available vouchers...</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <style>
            ...
            .voucher-card:hover {
                ...
                box-shadow: 0 12px 35px rgba(0, 53, 128, 0.15);
            }
            .voucher-banner {
    ...
    (function() {
        ...
        function switchVoucherTab(tab) {
            ...
            if (tab === 'collected') {
                ...
                btnCollected.style.color = '#003580';
                btnCollected.style.borderBottom = '2px solid #003580';
                btnCollected.style.fontWeight = '700';
                btnAvailable.style.color = '#94a3b8';
                btnAvailable.style.borderBottom = '2px solid transparent';
                btnAvailable.style.fontWeight = '600';
            } else {
                collected.style.display = 'none';
                available.style.display = '';
                btnAvailable.style.color = '#003580';
                btnAvailable.style.borderBottom = '2px solid #003580';
                btnAvailable.style.fontWeight = '700';
                btnCollected.style.color = '#94a3b8';
                btnCollected.style.borderBottom = '2px solid transparent';
                btnCollected.style.fontWeight = '600';
        ...
        function buildVoucherCard(v, mode) {
            ...
            if (mode === 'collected') {
                ...
                footerHtml = `
                    ...
                    <span style="font-size: 0.75rem; color: #94a3b8;">Expires: ${endDate}</span>
                `;
            } else {
                ...
                footerHtml += `<span style="font-size: 0.75rem; color: #94a3b8;">Expires: ${endDate}</span>`;
            }
            ...
            return `
                <div class="voucher-card">
                    ...
                    <div class="voucher-body">
                        ...
                        <div style="font-size:0.75rem;color:#94a3b8;"><i class="fas fa-bullseye" style="font-size:0.7rem;"></i> Valid for: ${targets}</div>
        ...
        function loadMyVouchers() {
            ...
            container.innerHTML = `<div style="text-align:center;color:#94a3b8;padding:40px;grid-column:1/-1;">
                <i class="fas fa-spinner fa-spin" style="font-size:2rem;"></i><p style="margin-top:12px;">Loading your vouchers...</p>
            </div>`;

            fetch('../api/user_voucher_api.php?action=get_my_vouchers')
                .then(r => r.json())
                .then(res => {
                    if (!res.success) { container.innerHTML = `<div style="grid-column:1/-1;text-align:center;color:#ef4444;padding:30px;">${res.message || 'Failed to load.'}</div>`; return; }
                    if (!res.data || res.data.length === 0) {
                        container.innerHTML = `
                            <div style="grid-column:1/-1;text-align:center;padding:50px;color:#64748b;">
                                <i class="fas fa-wallet" style="font-size:3rem;color:#cbd5e1;margin-bottom:16px;display:block;"></i>
                                <p style="font-weight:700;font-size:1.05rem;margin:0;">No vouchers in your wallet yet.</p>
                                <p style="font-size:0.85rem;color:#94a3b8;margin-top:4px;">Switch to "Available to Claim" to grab some deals!</p>
                            </div>`;
                        return;
                    }
                ...
                .catch(() => {
                    container.innerHTML = `<div style="grid-column:1/-1;text-align:center;color:#ef4444;padding:30px;">Error connecting to server.</div>`;
                });
        }
        ...
        function loadAvailableVouchers() {
            ...
            container.innerHTML = `<div style="text-align:center;color:#94a3b8;padding:40px;grid-column:1/-1;">
                <i class="fas fa-spinner fa-spin" style="font-size:2rem;"></i><p style="margin-top:12px;">Loading available vouchers...</p>
            </div>`;
            ...
            fetch('../api/user_voucher_api.php?action=get_available_vouchers')
                ...
                .then(res => {
                    ...
                    if (!res.data || res.data.length === 0) {
                        container.innerHTML = `
                            <div style="grid-column:1/-1;text-align:center;padding:50px;color:#64748b;">
                                <i class="fas fa-gift" style="font-size:3rem;color:#cbd5e1;margin-bottom:16px;display:block;"></i>
                                <p style="font-weight:700;font-size:1.05rem;margin:0;">No vouchers available right now.</p>
                                <p style="font-size:0.85rem;color:#94a3b8;margin-top:4px;">Check back later for exclusive deals!</p>
                            </div>`;
                        return;
                    }

<?php
// Exclude chatbot directly on View Details pages mapped by URL params
if (isset($_GET['dest']) || isset($_GET['id']) || isset($_GET['package']) || isset($_GET['deal']) || isset($_GET['cruise_id']) || isset($_GET['flight_id'])) {
    return;
}

$hd_base_img = 'images/Heydream Logo.png';
$hd_api_base = 'inquiry/';
if (file_exists('../images/Heydream Logo.png')) {
    $hd_base_img = '../images/Heydream Logo.png';
    $hd_api_base = '../inquiry/';
} elseif (file_exists('../../images/Heydream Logo.png')) {
    $hd_base_img = '../../images/Heydream Logo.png';
    $hd_api_base = '../../inquiry/';
}
?>
...
<style>
    ...
    .hd-social-footer-icons a:nth-child(1):hover {
        background: #1877f2;
    }

    .hd-social-footer-icons a:nth-child(2):hover {
        background: #e1306c;
    }

    .hd-social-footer-icons a:nth-child(3):hover {
        background: #1da1f2;
    }

    .hd-social-footer-icons a:nth-child(4):hover {
        background: #000000;
    }
    ...
    @keyframes hdPulse {
        ...
        0%,
        100% {
            box-shadow: 0 6px 24px rgba(0, 53, 128, 0.45), 0 0 0 0 rgba(0, 53, 128, 0.3);
        }

        50% {
            box-shadow: 0 6px 24px rgba(0, 53, 128, 0.45), 0 0 0 10px rgba(0, 53, 128, 0);
        }
    }
    ...
    .hd-typing-dots span {
        ...
        background: #94a3b8;
        border-radius: 50%;
        animation: hdDot 1.2s infinite;
    }

    .hd-typing-dots span:nth-child(2) {
        animation-delay: 0.2s;
    }

    .hd-typing-dots span:nth-child(3) {
    ...
    @media (max-width: 768px) {
        ...
        #hd-chat-window {
            position: fixed;
            top: 65px;
            /* Clear the 50px navbar header + 15px margin */
            bottom: 85px;
            left: 10px;
            right: 10px;
            width: auto;
    ...
    @media (max-width: 480px) {
        ...
        #hd-chat-window {
            position: fixed;
            top: 62px;
            /* Clear the 50px navbar header + 12px margin */
            bottom: 78px;
            left: 6px;
            right: 6px;
            width: auto;
...
<button id="hd-chat-bubble" onclick="hdToggleChat()" title="Chat with HeyDream AI">
    <img src="<?php echo htmlspecialchars($hd_base_img); ?>" id="hd-bubble-logo" alt="HeyDream AI">
...
<div id="hd-chat-window">
    ...
    <div class="hd-chat-header">
        <div class="hd-chat-avatar"><img src="<?php echo htmlspecialchars($hd_base_img); ?>" alt="HeyDream"></div>
        <div class="hd-chat-header-info">
            <strong>Customer Service</strong>
            <span><span class="hd-online-dot"></span>Online</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
            <button class="hd-new-chat-btn" id="hd-new-chat-btn" onclick="hdStartNewChat()" title="Start New Chat"
                style="background: rgba(255,255,255,0.18); border: none; color: white; padding: 4px 8px; border-radius: 8px; font-size: 0.72rem; font-weight: 700; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 4px; outline: none; margin-right: 4px;"><i
                    class="fas fa-plus"></i> New Chat</button>
            <button class="hd-menu-btn" id="hd-menu-trigger" onclick="hdToggleMenu()" title="Menu"><i
    ...
    <div class="hd-chat-input-area" style="position: relative; display: flex; align-items: center; gap: 8px;">
        <button id="hd-emoji-btn" onclick="hdToggleEmojiPicker(event)" title="Add Emoji"
            style="background: none; border: none; color: #64748b; font-size: 1.25rem; cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 0 2px; transition: all 0.2s; outline: none; flex-shrink: 0;"><i
                class="far fa-smile"></i></button>
        <input type="text" id="hd-chat-input" placeholder="Ask me anything about travel..." maxlength="300"
            style="flex: 1; min-width: 0;">
        <button id="hd-send-btn" onclick="hdSendMessage()" style="flex-shrink: 0;"><i
                class="fas fa-paper-plane"></i></button>

        <!-- EMOJI PICKER POPUP -->
        <div id="hd-emoji-picker"
            style="display: none; position: absolute; bottom: 55px; left: 10px; background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(10px); border: 1.5px solid #e2e8f0; border-radius: 16px; box-shadow: 0 10px 30px -5px rgba(0, 53, 128, 0.15); width: 220px; padding: 12px; z-index: 1000; animation: hdEmojiIn 0.22s cubic-bezier(0.34, 1.56, 0.64, 1);">
            <div
                style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 8px; font-size: 1.15rem; text-align: center; user-select: none;">
                <span onclick="hdInsertEmoji('😊')"
                ...
                <span onclick="hdInsertEmoji('🌴')"
                    ...
                    onmouseover="this.style.transform='scale(1.25)'"
                    onmouseout="this.style.transform='scale(1)'">🌴</span>
                <span onclick="hdInsertEmoji('🗺️')"
                    style="cursor: pointer; transition: transform 0.1s; display: inline-block;"
                    onmouseover="this.style.transform='scale(1.25)'"
                    onmouseout="this.style.transform='scale(1)'">🗺️</span>
                <span onclick="hdInsertEmoji('🏨')"
                    style="cursor: pointer; transition: transform 0.1s; display: inline-block;"
                    onmouseover="this.style.transform='scale(1.25)'"
                    onmouseout="this.style.transform='scale(1)'">🏨</span>
...
<script>
    ...
    const hdQuickReplies = [
        ...
        "Do you have local tour packages?",
    ...
    let hdSessionId = localStorage.getItem('hd_index_chat_session_id');
    if (!hdSessionId) {
        hdSessionId = 'hd_idx_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now();
        localStorage.setItem('hd_index_chat_session_id', hdSessionId);
    }
    ...
    function hdRequestLiveAgent() {
        ...
        fetch('<?php echo htmlspecialchars($hd_api_base); ?>ai_chat.php', {
            method: 'POST',
    ...
    function hdAddMsg(role, html, isPolling = false, msgId = null) {
        ...
        const avatarImg = (role === 'admin') ? '<?php echo htmlspecialchars($hd_base_img); ?>' : '<?php echo htmlspecialchars($hd_base_img); ?>';
    ...
    function hdShowTyping() {
        ...
        t.id = 'hd-typing';
        t.innerHTML = `<div class="hd-msg-avatar"><img src="<?php echo htmlspecialchars($hd_base_img); ?>" alt="HD"></div><div class="hd-msg-bubble"><div class="hd-typing-dots"><span></span><span></span><span></span></div></div>`;
    ...
    async function hdSendMessage(text, isSuggestion = false) {
        ...
        try {
            const res = await fetch('<?php echo htmlspecialchars($hd_api_base); ?>ai_chat.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: msg,
            ...
            if (data.status === 'needs_client_call') {
                await hdCallGeminiDirectly(msg, data, '<?php echo htmlspecialchars($hd_api_base); ?>ai_chat.php', false);
                return;
            }
    ...
    async function hdInitLastMsgId() {
        try {
            const res = await fetch(`<?php echo htmlspecialchars($hd_api_base); ?>get_chat_updates.php?session_id=${hdSessionId}&init=1`);
            const data = await res.json();
            if (data.success) {
                if (data.max_id) {
    ...
    const hdSidePanelObserver = new MutationObserver(() => {
        let panelOpen = false;
        document.querySelectorAll('.side-panel, #side-panel, .details-modal, #details-modal').forEach(el => {
            if (el.classList.contains('open') || el.classList.contains('active') || el.style.display === 'block' || el.style.transform.includes('translateX(0)')) {
                panelOpen = true;
            }
        });

        const bubble = document.getElementById('hd-chat-bubble');
        const hdWindow = document.getElementById('hd-chat-window');

        if (panelOpen) {
            if (bubble) bubble.style.display = 'none';
            if (hdWindow) hdWindow.style.display = 'none';
        } else {
            if (bubble) bubble.style.display = 'flex';
            if (hdWindow) hdWindow.style.display = '';
        }
    });
    hdSidePanelObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'style'] });
    // Global helper for outside triggering
    window.toggleHeyDreamChatbot = hdToggleChat;

    // Close chat when clicking outside
    document.addEventListener('click', function (e) {
        const chatWindow = document.getElementById('hd-chat-window');
        const chatBubble = document.getElementById('hd-chat-bubble');

        if (typeof hdChatOpen !== 'undefined' && hdChatOpen && chatWindow && chatBubble) {
            if (chatWindow.contains(e.target) || chatBubble.contains(e.target)) return;

            let target = e.target;
            while (target && target !== document) {
                if (target.hasAttribute && target.hasAttribute('onclick')) {
                    let onclickVal = target.getAttribute('onclick') || '';
                    if (onclickVal.includes('toggleHeyDreamChatbot') || onclickVal.includes('openSupportChat') || onclickVal.includes('hdToggleChat')) {
                        return;
                    }
                }

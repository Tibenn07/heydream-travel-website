...
window.selectHomeHotel = function (index, name, stars, price) {
    ...
    document.querySelectorAll('#homeHotelDropdown .hotel-option').forEach((el, i) => {
    ...
    let html = `
        ...
        <div id="homeDetailsView">
            ...
            <div id="home-pane-info" class="home-pane" style="display:block;">
                <div class="package-details-grid" style="margin-bottom:15px;">
                    ...
                    <div class="detail-item hotel-selection-item" onclick="toggleHomeHotelSelection()" style="cursor:pointer;">
                        <i class="fas fa-hotel"></i>
                        <div class="detail-label">HOTEL</div>
                        <div class="detail-value" id="homeSelectedHotelName" style="color:#ff9800; font-weight:bold;">${destination.hotels && destination.hotels.length > 0 ? escapeHtml(destination.hotels[0].name) + (destination.hotels[0].stars ? '⭐'.repeat(destination.hotels[0].stars) : '') : 'Change Hotel'} <i class="fas fa-chevron-down" style="font-size:0.7rem; margin-left:5px;"></i></div>
                    </div>
                </div>
                <div id="homeHotelDropdown" class="hotel-dropdown" style="display:none; margin: 10px 0; background: white; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                    ${(destination.hotels || []).map((h, i) => `
                        <div class="hotel-option ${i === 0 ? 'active' : ''}" onclick="selectHomeHotel(${i}, '${escapeHtml(h.name).replace(/'/g, "\\'")}', ${h.stars || 0}, ${h.price})" style="padding: 12px 15px; border-bottom: 1px solid #eee; cursor: pointer; transition: background 0.2s;">
        ...
        <div id="homeBookingView" style="display:none;">
            <div style="margin-bottom:15px;">
                <button onclick="document.getElementById('homeBookingView').style.display='none'; document.getElementById('homeDetailsView').style.display='block';" style="background:none; border:none; color:#003580; font-weight:600; cursor:pointer;"><i class="fas fa-arrow-left"></i> Back to Details</button>
            </div>
...
window.resumeHomeBooking = function (destKey, step) {
    const modal = document.getElementById('homeDestinationModal');
